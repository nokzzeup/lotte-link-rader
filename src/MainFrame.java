...
public class MainFrame extends JFrame {
    ...
    private JButton wishBtn; // 찜기능 추가
    public MainFrame() {
    ...
    private void startBadgeRefresh() {
        ...
        badgeRefreshTimer.schedule(new TimerTask() {
            @Override public void run() { SwingUtilities.invokeLater(() -> refreshAllBadges()); }
        }, 15000, 15000);
    }
        ...
        for (String[] game : GameSchedule.getUpcomingGames(30)) {
            ...
            new Thread(() -> {
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    boolean sold = seats.stream().filter(s -> !s.getName().contains("휠체어")).allMatch(s -> s.getRemain() == 0);
    ...
    private JPanel createLeftPanel() {
        ...
        tabs.setBorder(new MatteBorder(0, 0, 1, 0, new Color(229, 231, 235)));
    ...
    private JPanel createGameItem(String[] game, String status) {
        ...
        item.addMouseListener(new MouseAdapter() {
            @Override public void mouseEntered(MouseEvent e) {
                if (selectedItem != wrapper) setItemBg(wrapper, item, info, badges, dateBox, new Color(249,250,251));
            }
            @Override public void mouseExited(MouseEvent e) {
                if (selectedItem != wrapper) setItemBg(wrapper, item, info, badges, dateBox, WHITE);
            }
            @Override public void mouseClicked(MouseEvent e) {
                // 이전 선택 초기화 - wrapper 안의 모든 컴포넌트 순회해서 배경 바꾸기
    ...
    private void showAwayMessage(String[] game) {
        ...
        JPanel p = buildMessagePanel(mo+"/"+dy+" vs "+game[2], null,
                "✈", "원정 경기입니다", "원정 경기는 잔여석 정보를 제공하지 않습니다.");
        seatScroll.setViewportView(p);
    ...
    private JPanel createSeatRow(SeatInfo seat) {
        ...
        Color barColor;
        if (remain == 0)      barColor = new Color(229,231,235);
        else if (remain < 30) barColor = new Color(251,191,36);
        else                  barColor = new Color(52,211,153);
        ...
        Color rc;
        String rt;
        if (remain == 0)      { rc = new Color(209,213,219); rt = "매진"; }
        else if (remain < 30) { rc = new Color(217,119,6);   rt = remain+"석"; }
        else                  { rc = new Color(5,150,105);    rt = remain+"석"; }
